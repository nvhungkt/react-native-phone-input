...
export default class PhoneInput extends Component {
  ...
  onChangePhoneNumber(displayNumber) {
    const number = displayNumber.replace(' ', '');
      ...
      if (countryData) {
        ...
        this.setState(
          ...
          () => {
            ...
            const currentDialCode = PhoneNumber.getDialCode(inputValue);
            const newInputValue = currentDialCode
              ? inputValue.replace(currentDialCode, `+${dialCode}`)
              : `+${dialCode}${inputValue.replace('+', '')}`;
            this.onChangePhoneNumber(newInputValue);
  ...
  render() {
    ...
    const dialCode = this.getCountryCode();
    const displayValue = dialCode ? inputValue.replace(dialCode, `${dialCode} `) : inputValue;
